...
const InboundLogs = ({ clientId }) => {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/client/inbound/logs?clientId=${clientId}`)
      .then(res => res.json())
      .then(setLogs);
  }, [clientId]);
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-xl font-semibold text-gray-900">Logs / Conversation</h3>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {logs.map((log, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{log.mobile}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <audio controls src={log.audioUrl} className="h-8" style={{ width: '120px' }} />
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <span className={`inline-block w-3 h-3 rounded-full mr-2 ${statusColors[log.leadStatus]}`}></span>
                      <span className={`text-sm font-medium capitalize ${statusTextColors[log.leadStatus]}`}>
