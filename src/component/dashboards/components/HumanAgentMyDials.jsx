...
function HumanAgentMyDials() {
  ...
  const fetchAssignedCampaigns = async () => {
    try {
      setLoading(true);
      setError("");
      const url = new URL(`https://aitotabackend-sih2.onrender.com/api/v1/human-agent/assigned-campaigns`);
  ...
  const fetchMyReport = async () => {
    try {
      ...
      const [reportRes, doneRes] = await Promise.all([
        fetch(`https://aitotabackend-sih2.onrender.com/api/v1/human-agent/dials/report${q ? `?${q}` : ""}`, {
          headers,
        }),
        fetch(`https://aitotabackend-sih2.onrender.com/api/v1/human-agent/dials/done${q ? `?${q}` : ""}`, {
          headers,
        }),
      ]);
  ...
  const saveDisposition = async (rowKey, contact) => {
    try {
      ...
      const res = await fetch(`https://aitotabackend-sih2.onrender.com/api/v1/human-agent/dials`, {
        method: "POST",
        headers: {
  ...
  const fetchAssignedContacts = async (page = 1) => {
    ...
    try {
      setLoading(true);
      setError("");
      const url = new URL(`https://aitotabackend-sih2.onrender.com/api/v1/human-agent/assigned-contacts`);
  ...
  const fetchAssignedGroups = async () => {
    try {
      setLoading(true);
      setError("");
      const res = await fetch(`https://aitotabackend-sih2.onrender.com/api/v1/human-agent/groups`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const json = await res.json();
      if (!res.ok || json?.success === false) {
  ...
  const fetchGroupContacts = async (groupIdParam) => {
    ...
    try {
      ...
      setError("");
      const res = await fetch(
        `https://aitotabackend-sih2.onrender.com/api/v1/human-agent/groups/${gid}/contacts`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const json = await res.json();
      if (!res.ok || json?.success === false) {
  ...
  const fetchAssignedGroupContacts = async (page = 1, filter = "") => {
    try {
      setLoading(true);
      setError("");
      const url = new URL(`https://aitotabackend-sih2.onrender.com/api/v1/human-agent/groups`);
  ...
  const fetchGroupContactsForDisplay = async (groupId) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(
        `https://aitotabackend-sih2.onrender.com/api/v1/human-agent/groups/${groupId}/contacts`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const json = await res.json();
      if (!res.ok || json?.success === false) {
